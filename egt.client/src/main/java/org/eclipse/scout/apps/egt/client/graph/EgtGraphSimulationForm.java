...
public class EgtGraphSimulationForm extends EgtGraphForm implements IEgtPageForm {
	...
	private void changeVisibilityOfFitnessColors() {
		...
		for (ITableRow row : getGraphDetailFormField().getInnerForm().getVerticesTabBox().getVerticesTableField().getTable()
				.getRows()) {
			IEgtSpeciesCode code = (IEgtSpeciesCode) BEANS.get(EgtSpeciesCodeType.class)
					.getCode(BEANS.get(EgtSpeciesCodeType.class).getCodeIdByEnum(getGraphDetailFormField().getInnerForm()
							.getVerticesTabBox().getVerticesTableField().getEgtGraphVertecColumn().getValue(row).getSpecies()));
		...
		for (ICode<Long> c : BEANS.get(EgtSpeciesCodeType.class).getCodes()) {
			if (species.contains(c)) {
				...
				double fitness = NumberUtility.nvl(getSimulationBox().getFitnessBox()
						.getFitnessColorBoxByCode((IEgtSpeciesCode) c).getFitnessField().getValue(), 0.0);
				getSimulationBox().getFitnessBox().getFitnessColorBoxByCode((IEgtSpeciesCode) c).getFitnessField()
						.setValue(fitness);
				getSimulationBox().getFitnessBox().getFitnessColorBoxByCode((IEgtSpeciesCode) c).getScaledFitnessField()
						.setEnabled(false);
			} else {
