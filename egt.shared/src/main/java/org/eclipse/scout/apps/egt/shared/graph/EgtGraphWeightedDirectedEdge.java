...
public class EgtGraphWeightedDirectedEdge {
	...
	private String buildSvgText() {
		...
		if (fromTo) {
			...
			svgText += "         d=\"M" + getFrom().getX() + "," + getFrom().getY() + " " + "\n" + "             L"
					+ ((getFrom().getX() * 0.7 + getTo().getX() * 0.3)) + "," + ((getFrom().getY() * 0.7 + getTo().getY() * 0.3))
					+ " " + "\n" + "             L" + getTo().getX() + "," + getTo().getY() + "\" " + "\n";
		}
		svgText += "   style=\"stroke: #0000cc; stroke-width: 1px; fill: none; marker-" + (fromTo ? "end" : "mid")
				+ ": url(#markerArrow);\" />" + "\n" + "  <text x=\"" + (fromTo ? (r * 3.2) : (r * 0.6))
				+ "\" y=\"0\" style=\"font-family: Arial;\"> " + "\n" + "    <textPath xlink:href=\"#" + getFrom().getId() + "-"
				+ getTo().getId() + "\" > " + "\n" + "        <tspan id=\"" + getFrom().getId() + "-" + getTo().getId()
				+ "-weight" + "\" dy=\"-10\">" + "\n" + "          " + getWeight() + "" + "\n" + "        </tspan>" + "\n"
				+ "    </textPath>" + "\n" + "  </text>";
		return svgText;
	}
